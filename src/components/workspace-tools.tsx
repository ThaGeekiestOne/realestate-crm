...
export function SettingsTool({ settings, setSettings, saveSettings, resetDemoData, canManage, back, notify }: { settings: WorkspaceSettings; setSettings: (settings: WorkspaceSettings) => void; saveSettings: (settings: WorkspaceSettings) => Promise<void>; resetDemoData?: () => void; canManage: boolean; back: () => void; notify: (message: string) => void }) {
  ...
  return <div>
    ...
    {resetDemoData && <section className="mt-4 rounded-2xl border border-[#f0d6d2] bg-white p-5"><h3 className="text-sm font-bold text-[#8d3f3d]">Reset demo workspace</h3><p className="mt-1 text-xs leading-5 text-[#87938e]">Remove browser-only Estate AI Flow changes and restore the seeded demo records.</p><button type="button" onClick={resetDemoData} className="mt-4 rounded-lg border border-[#e8c4bf] px-4 py-2.5 text-xs font-bold text-[#a34845]">Reset demo data</button></section>}
  </div>;
}
